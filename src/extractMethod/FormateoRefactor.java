...
public class FormateoRefactor {
	...
	public String calculaTexto(String texto) {
		...
		res = eliminarEspaciosPonerMayusculas(texto);

		// Después se eliminan las vocales
		res = eliminarVocales(res);

		// Finalmente se eliminan todos los caracteres no alfabéticos
		return eliminarCharsNoAlfabeticos(res);
	}

	private String eliminarCharsNoAlfabeticos(String res) {
		String txt3 = "";
	...
	private String eliminarVocales(String res) {
		String txt2 = "";
		for (int i = 0; i < res.length(); i++) {
	...
	private String eliminarEspaciosPonerMayusculas(String texto) {
		String res;
		String txt1 = "";
		txt1 = texto.replace(" ", "");
